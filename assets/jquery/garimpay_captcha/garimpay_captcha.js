...
$.extend(GarimPayCaptcha.prototype, {
	...
	_generateHTML: function(target, inst) {
		...
		var html = '<div class="' + this.propertyName + '-challenge" style="margin:7px 0 0 5px; float:right">' +
			'<div class="' + this.propertyName + '-text" style="border 1px solid #ccc">';
		...
		html += '</div><div class="' + this.propertyName + '-regen">' +
				'<img style="float:right; margin:3px 0 4px 0;" src="assets/images/captcha_reload.png" title="Carregar um novo código"></div></div>' + 		        
				'<input type="hidden" class="' + this.propertyName + '-hash" name="' +
...
/* Attach the real person functionality to a jQuery selection.
   ...
   @return  (jQuery) for chaining further calls or
            (any) getter value */
$.fn.garimpaycaptcha = function(options) {
...
var plugin = $.garimpaycaptcha = new GarimPayCaptcha(); // Singleton instance

$(document).on('click', 'div.' + plugin.propertyName + '-challenge', function() {
	if (!$(this).hasClass(plugin.propertyName + '-disabled')) {
		$(this).nextAll('.' + plugin.markerClassName).garimpaycaptcha('option', {});
	}
});
